...
const enviarRecordatorioPersonalizado = async (fechaNormalizada, intervaloPersonalizado, email, telefono, reminderData) => {
  const intervalMs = intervaloPersonalizado === "2min" ? 2 * 60 * 1000 : 2 * 60 * 60 * 1000;
...
const crearRecordatorio = async (req, res) => {
  try {
    ...
    const {
      titulo, fecha, descripcion, frecuencia, tipo,
      dosis, unidad, cantidadDisponible, intervaloPersonalizado,
    } = req.body;

    const info = await InfoUser.findOne({ userId });
    const user = await User.findById(userId);
    const email = info?.email || (/\S+@\S+\.\S+\.\S+/.test(user?.username) ? user.username : null);
    const telefono = info?.phone || null;

    if (!email && !telefono) return res.status(400).json({ message: "Usuario sin correo o teléfono válido" });

    const fechaNormalizada = fecha ? new Date(fecha) : new Date();
    const nombreCompleto = info?.name ? `${info.name} ${info.lastName || ''}`.trim() : "Paciente";

    const reminderData = {
      tipo, titulo, descripcion, frecuencia, dosis,
      unidad, cantidadDisponible, nombrePersona: nombreCompleto,
    };

    let reminder = new Reminder({
      userId, tipo, titulo, fecha: fechaNormalizada, descripcion,
      frecuencia, horarios: [], dosis, unidad, cantidadDisponible,
    });

    if (frecuencia === "Personalizada" && intervaloPersonalizado) {
      reminder.horarios = await enviarRecordatorioPersonalizado(fechaNormalizada, intervaloPersonalizado, email, telefono, reminderData);
      ...
      programarEnvio(frecuencia, [{ hour: hour24, minute }], fechaNormalizada, email, telefono, reminderData, tipo);
    }
...
// 📌 Marcar recordatorio completado
